...
func TestFader_Fade(t *testing.T) {
	tests := []struct {
		...
		{name: "start", pos: 0, bl: 5000, want: 0.0066928509242848, wantStart: true},
		{name: "start 10", pos: 10, bl: 5000, want: 0.007323641200864822, wantStart: true},
		{name: "start 1000", pos: 1000, bl: 5000, want: 0.9832142177368054, wantStart: true},
		{name: "start 1120", pos: 1120, bl: 5000, want: 1.0, wantStart: true},
		{name: "middle", pos: 2000, bl: 5000, want: 1.0, wantStart: true},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			f := defaultFader
			got, gotStart := f.Fade(tt.pos, tt.bl)
			if utils.Float64Equals(got, tt.want) == false {
				t.Errorf("Fade() = %v, want %v", got, tt.want)
			}
			if gotStart != tt.wantStart {
				t.Errorf("Fade() start = %v, want %v", gotStart, tt.wantStart)
			}
		})
	}
}
